  ...
  return (
    <div className="bg-[#e2edf4] rounded-[14px] p-4 sm:p-5 mb-4">
      ...
      <div className="flex flex-col gap-1 mb-2.5">
        ...
        <input type="text" placeholder="Full name" value={data.fullName} onChange={update('fullName')} className={inputCls} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-2.5">
        <div className="flex flex-col gap-1">
          <label className={labelCls}>Phone</label>
          <input type="text" placeholder="213+ -------" value={data.phone} onChange={update('phone')} className={inputCls} />
        </div>
        <div className="flex flex-col gap-1">
          <label className={labelCls}>Email</label>
          <input type="email" placeholder="you@example.com" value={data.email} onChange={update('email')} className={inputCls} />
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-2.5">
        <div className="flex flex-col gap-1">
          <label className={labelCls}>Birth Date</label>
          <input type="date" value={data.birthDate} onChange={update('birthDate')} className={inputCls} />
        </div>
        <div className="flex flex-col gap-1">
          <label className={labelCls}>National Nº</label>
          <input type="text" placeholder="e.g. 123456780123456" value={data.nationalId} onChange={update('nationalId')} className={inputCls} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-2.5">
        <div className="flex flex-col gap-1">
          ...
          <select value={data.wilaya} onChange={update('wilaya')} className={`${inputCls} appearance-none cursor-pointer pr-8`}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="text" placeholder="e.g. username#1234" value={data.discord} onChange={update('discord')} className={inputCls} />
        </div>
      </div>
      <div className="flex flex-col gap-1">
        <label className={labelCls}>T-shirt Size</label>
        <select value={data.tshirt} onChange={update('tshirt')} className={`${inputCls} appearance-none cursor-pointer pr-8`}>
            ...
            <form onSubmit={handleSubmit} className="font-['Montserrat',sans-serif]">
              <div className="bg-[#e2edf4] rounded-[14px] p-4 sm:p-5 mb-4">
                <div className="flex flex-col gap-1">
                  ...
                  <input type="text" placeholder="Your team name" value={teamName} onChange={(e) => setTeamName(e.target.value)} className={`${inputCls} !text-sm !py-3`} />
                </div>
              </div>
              ...
              <button type="submit" className="w-full py-4 rounded-xl text-white text-[15px] font-bold tracking-wide cursor-pointer mt-2 transition-all duration-300 hover:opacity-90 hover:-translate-y-0.5 font-['Montserrat',sans-serif]"
